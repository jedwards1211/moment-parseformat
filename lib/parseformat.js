...
var regexDayNames = new RegExp(bounded(dayNames.join('|')), 'i')
var regexAbbreviatedDayNames = new RegExp(bounded(abbreviatedDayNames.join('|')), 'i')
var regexShortestDayNames = new RegExp(bounded(shortestDayNames.join('|')), 'i')
var regexMonthNames = new RegExp(bounded(monthNames.join('|')), 'i')
var regexAbbreviatedMonthNames = new RegExp(bounded(abbreviatedMonthNames.join('|')), 'i')

var regexFirstSecondThirdFourth = /(\d+)(st|nd|rd|th)(?![a-z])/i
...
function parseFormat (dateString, options) {
  ...
  // Monday ☛ dddd
  format = format.replace(regexDayNames, '$1dddd')
  // Mon ☛ ddd
  format = format.replace(regexAbbreviatedDayNames, '$1ddd')
  // Mo ☛ dd
  format = format.replace(regexShortestDayNames, '$1dd')

  // MONTHS

  // January ☛ MMMM
  format = format.replace(regexMonthNames, '$1MMMM')
  // Jan ☛ MMM
  format = format.replace(regexAbbreviatedMonthNames, '$1MMM')
